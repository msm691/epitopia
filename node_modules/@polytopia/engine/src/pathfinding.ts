...
import { canEnterTile, chebyshev } from "./units.js";
  ...
  while (openSet.size > 0) {
    ...
    for (let dy = -1; dy <= 1; dy++) {
      for (let dx = -1; dx <= 1; dx++) {
        ...
        if (isWater && !canNavigate) continue;
...
export function hasContinuousRoad(state: GameState, start: Coord, goal: Coord, maxDist: number): boolean {
  if (chebyshev(start, goal) > maxDist) return false;
  ...
  const queue: { idx: number, dist: number }[] = [{ idx: startIdx, dist: 0 }];
