 ...
 */
import { FOUNDED_CITY_LEVEL, RESOURCE_POP_GAIN, SAGE_NAMES, VILLAGE_DENSITY, VILLAGE_MIN_SPACING, } from "@polytopia/shared";
...
const MAP_SIZE_BONUS = {
    terres: 0,
    continents: 2,
    archipel: 4,
};
export function mapSizeForPlayers(count, mapType) {
    let base;
    if (count <= 2)
        base = 16;
    else if (count <= 4)
 ...
 */
function buildLandMask(rng, width, height, playerCount, mapType) {
    switch (mapType) {
        case "continents":
...
function carveLakes(rng, mask, width, height) {
    ...
    for (let l = 0; l < count; l++) {
        ...
        while (frontier.length > 0 && carved < size) {
            ...
            if (!mask[i] || x <= 0 || y <= 0 || x >= width - 1 || y >= height - 1)
                continue;
            mask[i] = false;
            carved++;
            for (const [dx, dy] of NEIGHBORS4)
                frontier.push((y + dy) * width + (x + dx));
        }
    }
}
/**
 ...
 */
function growBlobAt(rng, land, width, height, cx, cy, size) {
    const cells = [];
 ...
 */
function buildArchipelagoMask(rng, width, height, playerCount, landFraction) {
    ...
    while (count < targetLand && guard-- > 0) {
        ...
        if (land[y * width + x] || landNeighbors8(land, width, height, x, y) > 0)
            continue;
...
function landNeighbors8(mask, width, height, x, y) {
    let n = 0;
 ...
 */
function pickSpreadAnchors(rng, width, height, count, margin) {
    const inset = (v, max) => Math.min(Math.max(v, margin), max - 1 - margin);
    const rand = () => ({
    ...
    for (let i = 1; i < count; i++) {
        ...
        for (let c = 0; c < 48; c++) {
            ...
            let minD = Infinity;
            for (const a of anchors)
                minD = Math.min(minD, distance(cand, a));
            if (minD > bestDist) {
                bestDist = minD;
                best = cand;
            }
 ...
 */
function buildContinentsMask(rng, width, height, landFraction) {
    const n = width * height;
    const targetLand = Math.round(n * landFraction);
    const land = new Array(n).fill(false);
    const frontier = [];
    let count = 0;
    const pushNeighbors = (i) => {
        const x = i % width;
        const y = (i / width) | 0;
        for (const [dx, dy] of NEIGHBORS4) {
            const nx = x + dx;
 ...
 */
function smoothCoasts(land, width, height) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            if (!src[i] && ln >= 5)
                land[i] = true; // trou d'eau cerné de terre -> terre
            else if (src[i] && ln <= 1)
                land[i] = false; // pointe de terre isolée -> eau
        }
    }
}
/** Sème quelques petites îles (2-3 cases) en pleine eau (points de conflit / villages). */
function addSmallIslands(rng, land, width, height) {
    ...
    for (let a = 0; a < maxAttempts && placed < target; a++) {
        ...
        const y = rng.int(1, height - 2);
        const i = y * width + x;
        if (land[i] || landNeighbors8(land, width, height, x, y) > 0)
            continue; // pleine eau
        land[i] = true;
 ...
 */
function placeBiomeClusters(rng, tiles, width, height, landIndices, terrain, targetTiles) {
    if (targetTiles <= 0 || landIndices.length === 0)
        return;
    let converted = 0;
...
export function generateMap(seed, width, height, playerCount, mapType = "terres") {
    ...
    const { mask: landMask, starts: forcedStarts } = buildLandMask(rng, width, height, playerCount, mapType);
    const tiles = [];
    const landIndices = [];
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
    ...
    placeBiomeClusters(rng, tiles, width, height, landIndices, "foret", Math.round(landIndices.length * FOREST_LAND_FRACTION));
    placeBiomeClusters(rng, tiles, width, height, landIndices, "montagne", Math.round(landIndices.length * MOUNTAIN_LAND_FRACTION));
    const land = tiles
    ...
    const mainland = land.filter((c) => landNeighbors8(landMaskNow, width, height, c.x, c.y) >= MAINLAND_MIN_LAND_NEIGHBORS);
    ...
    const villageDensity = mapType === "archipel"
 ...
 */
function placeSages(rng, tiles, width, height, starts) {
    const startKeys = new Set(starts.map((s) => `${s.x},${s.y}`));
    ...
    const candidates = tiles.filter((t) => t.terrain === "champ" &&
    ...
    const villages = tiles.filter((t) => t.village).map((t) => ({ x: t.x, y: t.y }));
    const placed = [];
    const farEnough = (c) => {
        for (const s of starts)
        ...
        for (const v of villages)
            if (chebyshevLocal(c, v) < VILLAGE_MIN_SPACING)
                return false;
    ...
    for (let i = 0; i < maxAttempts && placed.length < SAGE_NAMES.length; i++) {
        ...
        if (cand.sage || cand.village || !farEnough(c))
            continue;
...
function placeNaturalWonders(rng, tiles, width, height, starts) {
    // Volcan (3 montagnes)
 ...
 */
function ensureVillageResources(tiles, width, height, villages, startKeys) {
    for (const v of villages) {
        const ring = [];
        for (let dy = -1; dy <= 1; dy++) {
            for (let dx = -1; dx <= 1; dx++) {
 ...
 */
function placeVillages(_rng, tiles, width, height, starts, target) {
    const startKeys = new Set(starts.map((s) => `${s.x},${s.y}`));
    const land = tiles.filter((t) => isLandTerrain(t.terrain) &&
        t.resource === undefined &&
        !startKeys.has(`${t.x},${t.y}`));
    if (land.length === 0 || target <= 0 || starts.length === 0)
        return [];
    const placed = [];
    ...
    const isValid = (cand) => !cand.village && farEnough(cand) && ringLand(cand) >= VILLAGE_TARGET_POP;
    const mark = (cand, owner) => {
        cand.terrain = "champ";
        cand.village = true;
...
function placeRuins(rng, tiles, starts) {
    const target = Math.floor(tiles.length / 100);
    const startKeys = new Set(starts.map((s) => `${s.x},${s.y}`));
    let placed = 0;
    for (let i = 0; i < tiles.length && placed < target; i++) {
        const t = tiles[rng.int(0, tiles.length - 1)];
        if (!t || !isLandTerrain(t.terrain) || t.village || t.sage || startKeys.has(`${t.x},${t.y}`))
            continue;
        // Éloigné des départs
        let far = true;
        for (const s of starts) {
            if (chebyshevLocal(t, s) < 4)
                far = false;
        }
        if (far) {
            t.ruin = true;
...
function placeBarbarianCamps(rng, tiles, starts) {
    const target = Math.floor(tiles.length / 120);
    const startKeys = new Set(starts.map((s) => `${s.x},${s.y}`));
    let placed = 0;
    for (let i = 0; i < tiles.length && placed < target; i++) {
        const t = tiles[rng.int(0, tiles.length - 1)];
        if (!t || !isLandTerrain(t.terrain) || t.village || t.sage || t.ruin || startKeys.has(`${t.x},${t.y}`))
            continue;
        // Très éloigné des départs
        let far = true;
        for (const s of starts) {
            if (chebyshevLocal(t, s) < 5)
                far = false;
        }
        if (far) {
