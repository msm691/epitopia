 ...
 */
import { SAGE_MAX_STARS, SAGE_MIN_STARS, SAGE_STAR_FACTOR, TECHS, UNIT_NAMES, UNIT_STATS, WALL_MAX_HP, } from "@polytopia/shared";
...
function illumination(state, pid) {
    const owned = new Set(state.players[pid]?.unlockedTechs ?? []);
    const candidates = Object.values(TECHS).filter((t) => !owned.has(t.id) && (t.requires === null || owned.has(t.requires)));
    ...
    return {
        state: {
            ...state,
            players: state.players.map((p) => p.id === pid ? { ...p, unlockedTechs: [...p.unlockedTechs, tech.id] } : p),
        },
        title: "Illumination",
        detail: `Technologie « ${tech.name} » offerte.`,
    };
}
function fortification(state, pid, at) {
    const mine = state.cities.filter((c) => c.ownerId === pid && !c.hasWall);
    if (mine.length === 0)
        return null;
    let best = mine[0];
    for (const c of mine)
        if (chebyshev(at, c) < chebyshev(at, best))
            best = c;
    return {
        state: {
            ...state,
            cities: state.cities.map((c) => c.id === best.id ? { ...c, hasWall: true, wallHp: WALL_MAX_HP } : c),
        },
        title: "Fortification",
...
function applyBonus(state, pid, pick, amount, at) {
...
// --- MALUS ------------------------------------------------------------------
function desertion(state, pid) {
    const mine = state.units.filter((u) => u.ownerId === pid);
    if (mine.length === 0)
        return null;
    let best = mine[0];
    for (const u of mine)
        if (UNIT_STATS[u.type].cost > UNIT_STATS[best.type].cost)
            best = u;
...
function racket(state, pid, amount) {
...
function applyMalus(state, pid, pick, amount) {
