...
import { DEFENSE_BONUS_CITY, DEFENSE_BONUS_WALL, IMPROVEMENT_COSTS, RESOURCE_POP_GAIN, TREASURE_STARS, UNIT_STATS, WALL_MAX_HP, } from "@polytopia/shared";
...
function gridState(size, playerCount) {
    const tiles = [];
    for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++)
            tiles.push({ x, y, terrain: "champ" });
    }
    return {
        width: size,
        height: size,
...
describe("generateMap — types de carte & biomes", () => {
    ...
    function landNeighborsAround(tiles, w, h, x, y) {
        let n = 0;
        for (let dy = -1; dy <= 1; dy++) {
            for (let dx = -1; dx <= 1; dx++) {
                ...
                if (nx < 0 || ny < 0 || nx >= w || ny >= h)
                    continue;
                if (isLandTerrain(tiles[ny * w + nx].terrain))
                    n++;
            }
        }
        return n;
                ...
                for (let dy = -1; dy <= 1; dy++) {
                    for (let dx = -1; dx <= 1; dx++) {
                        ...
                        if (nx < 0 || ny < 0 || nx >= 18 || ny >= 18)
                            continue;
                        if (tiles[tileIndex(18, nx, ny)].resource !== undefined)
                            count++;
                    }
                }
                ...
                for (let dy = -1; dy <= 1; dy++) {
                    for (let dx = -1; dx <= 1; dx++) {
                        ...
                        if (nx < 0 || ny < 0 || nx >= 20 || ny >= 20)
                            continue;
                        if (tiles[tileIndex(20, nx, ny)].resource !== undefined)
                            count++;
                    }
                }
                expect(count).toBeGreaterThanOrEqual(2);
...
describe("actions non implémentées", () => {
    it("FOUND_CITY est illégal pour l'instant", () => {
        ...
        expect(() => applyAction(s, { type: "FOUND_CITY", unitId: "u1" })).toThrow(IllegalActionError);
    });
});
...
describe("computeCombat (formule Polytopia)", () => {
    const guerrier = (hp) => makeUnit("g", "guerrier", 0, 0, 0, false, hp);
    const archer = (hp) => makeUnit("a", "archer", 0, 0, 0, false, hp);
...
describe("HARVEST_RESOURCE (2c)", () => {
    ...
    it("est illégal : ressource nécessitant une tech non possédée", () => {
        ...
        expect(isLegal(s, { type: "HARVEST_RESOURCE", cityId: "c0", at: { x: 3, y: 2 } })).toBe(false);
    });
    it("est illégal : hors du rayon, sans ressource, ou trop pauvre", () => {
        const s = harvestState();
        s.tiles[tileIndex(5, 4, 4)].resource = "fruits"; // hors rayon 1
        expect(isLegal(s, { type: "HARVEST_RESOURCE", cityId: "c0", at: { x: 4, y: 4 } })).toBe(false);
        ...
        expect(isLegal(poor, { type: "HARVEST_RESOURCE", cityId: "c0", at: { x: 3, y: 2 } })).toBe(false);
    });
});
...
describe("IA gloutonne (4a)", () => {
    ...
    it("avec Navigation, embarque pour traverser l'eau vers l'ennemi", () => {
        const s = aiState(5);
        s.players = s.players.map((p) => p.id === 0 ? { ...p, unlockedTechs: ["peche", "navigation"] } : p);
        for (let y = 0; y < 5; y++)
            s.tiles[tileIndex(5, 2, y)].terrain = "eau"; // bras de mer x=2
    ...
    it("sans Navigation mais bordée d'eau, recherche d'abord la marine (Pêche)", () => {
        ...
        for (let i = 0; i < 4; i++)
            placeUnit(s, makeUnit(`g${i}`, "guerrier", 0, i, 4, true));
    ...
    it("sans Navigation, ne traverse jamais l'eau (reste à terre)", () => {
        ...
        for (let y = 0; y < 5; y++)
            s.tiles[tileIndex(5, 2, y)].terrain = "eau";
        activeWarrior(s, "u0", 0, 1, 2);
        placeUnit(s, makeUnit("foe", "guerrier", 1, 3, 2, false));
        const a = nextAIAction(s, 0);
        if (a.type === "MOVE_UNIT") {
...
describe("posture IA (anti-rush, Étape D)", () => {
    function withCity(s, id, owner, x, y) {
        s.cities.push({ id, ownerId: owner, x, y, level: 1, population: 0, starsPerTurn: 2 });
        s.tiles[tileIndex(s.width, x, y)].cityId = id;
        s.tiles[tileIndex(s.width, x, y)].ownerId = owner;
    }
    function richP0(s, stars) {
        s.players = s.players.map((p) => (p.id === 0 ? { ...p, stars } : p));
    }
            ...
            for (const v of villages) {
                ...
                for (let dy = -1; dy <= 1; dy++) {
                    for (let dx = -1; dx <= 1; dx++) {
                        ...
                        if (t?.resource)
                            pop += RESOURCE_POP_GAIN[t.resource];
                    }
                }
...
describe("Récompenses de montée de niveau", () => {
    ...
    it("illégal sans choix en attente, pour un non-propriétaire, ou troupe sans place", () => {
        expect(isLegal(rewardCity(0), { type: "CLAIM_CITY_REWARD", cityId: "c0", reward: "atelier" })).toBe(false);
        const notMine = rewardCity(1);
        notMine.cities[0].ownerId = 1;
        expect(isLegal(notMine, { type: "CLAIM_CITY_REWARD", cityId: "c0", reward: "atelier" })).toBe(false);
        ...
        expect(isLegal(packed, { type: "CLAIM_CITY_REWARD", cityId: "c0", reward: "troupe" })).toBe(false);
        // ...mais l'atelier reste encaissable.
        expect(isLegal(packed, { type: "CLAIM_CITY_REWARD", cityId: "c0", reward: "atelier" })).toBe(true);
    });
...
describe("Territoire d'exploitation (agrandissement auto puis au choix)", () => {
    ...
    it("la récompense « agrandir » n'est dispo qu'après l'auto, et plafonne", () => {
        const s = freeCityState(0);
        s.cities[0].rewardsToPick = 1;
        const claim = { type: "CLAIM_CITY_REWARD", cityId: "c0", reward: "agrandir" };
...
describe("Construction (BUILD_IMPROVEMENT) — puits à étoiles", () => {
    ...
    function cityWithConstruction(stars) {
        const s = freeCityState(stars);
        s.players = s.players.map((p) => p.id === 0 ? { ...p, unlockedTechs: ["agriculture", "construction"] } : p);
        return s;
    }
...
describe("Siège : le rempart bloque l'entrée/capture jusqu'à destruction", () => {
    ...
    function walledEnemyCity(wallHp) {
        const s = gridState(5, 2);
        s.cities.push({
            id: "c1",
            ownerId: 1,
...
describe("équilibrage des unités (#8) — rééquilibrage tactique", () => {
    it("aucune unité n'a une défense de 0 (sinon dégâts maximaux / one-shots)", () => {
        for (const t of Object.keys(UNIT_STATS)) {
...
describe("temps de production des grosses unités (#11)", () => {
    ...
    function twoCityState(stars, techs) {
        const s = freeCityState(stars);
...
describe("Sages mystérieux (Stan & Nico, #9)", () => {
    it("generateMap place 2 sages sur la terre, espacés des départs", () => {
        ...
        for (const s of sages) {
            ...
            for (const st of starts)
                expect(cheb(s, st)).toBeGreaterThanOrEqual(3);
            for (const v of villages)
                expect(cheb(s, v)).toBeGreaterThanOrEqual(3);
            for (const o of sages)
                if (o !== s)
                    expect(cheb(s, o)).toBeGreaterThanOrEqual(3);
        }
    });
    /** Grille avec un sage en (3,3), une unité du joueur 0 adjacente, et 2 villes. */
    function sageState(seed) {
    ...
    it("applique un VRAI effet (bonus ou malus) selon le tirage, sur 30 seeds", () => {
        ...
        for (let seed = 1; seed <= 30; seed++) {
            ...
            if (n.lastSage.good) {
                good++;
                const changed = after.stars > before.stars ||
