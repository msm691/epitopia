...
import type { Coord, Resource, Terrain, Tile } from "@polytopia/shared";
...
const TERRAIN_WEIGHTS: ReadonlyArray<readonly [Terrain, number]> = [
  ["champ", 0.5],
  ["foret", 0.22],
  ["montagne", 0.16],
  ["eau", 0.12],
 ...
 */
export function mapSizeForPlayers(count: number): number {
...
function rollTerrain(rng: Rng): Terrain {
  const r = rng.next();
  let acc = 0;
...
function ensureStartResources(
  ...
  for (const t of neighbors) {
    if (count >= min) break;
    if (t.resource !== undefined) continue;
...
function placeVillages(
  ...
  for (let i = 0; i < maxAttempts && placed.length < target; i++) {
    ...
    if (cand.village || !farEnough(c)) continue;
    cand.terrain = "champ";
    cand.village = true;
    placed.push(c);
  }
