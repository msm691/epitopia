...
function removeUnit(units: Unit[], tiles: Tile[], width: number, dead: Unit): {
  units: Unit[];
  tiles: Tile[];
} {
...
function attack(state: GameState, action: AttackAction): GameState {
  ...
  const defenderNow: Unit = { ...defender, hp: defender.hp - result.defenderDamage };
...
function moveUnit(state: GameState, action: MoveUnitAction): GameState {
  ...
  const moved: Unit = { ...unit, x: to.x, y: to.y, hasMoved: true };
...
function endTurn(state: GameState): GameState {
  ...
  const players = state.players.map((p) =>
    p.id === nextPlayer ? { ...p, stars: p.stars + income } : p,
  );
