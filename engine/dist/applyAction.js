 ...
 */
import { CITY_HARVEST_RADIUS, FOUNDED_CITY_LEVEL, improvementCost, REWARD_TROOP_UNIT, RESOURCE_HARVEST_COST, RESOURCE_POP_GAIN, TREASURE_STARS, UNIT_STATS, unitBuildTurns, WALL_MAX_HP, DOCTRINES, } from "@polytopia/shared";
...
function progressQuest(state, pid, qType) {
    const p = state.players[pid];
    if (!p || !p.activeQuest || p.activeQuest.type !== qType)
        return state;
    ...
    if (nextProgress >= quest.target) {
        ...
        if (quest.reward === "stars") {
            ...
            const candidates = Object.values(import("@polytopia/shared").then(m => m.TECHS).catch(() => ({}))); // HACK: TECHS is imported from shared
...
export function applyAction(state, action) {
    ...
    switch (action.type) {
        ...
        case "UPGRADE_HERO":
            return upgradeHero(state, action);
    }
}
function adoptDoctrine(state, action) {
    const doctrine = DOCTRINES[action.doctrineId];
    const players = state.players.map((p) => p.id === state.currentPlayer
        ? {
            ...p,
...
function proposePeace(state, action) {
    const peaceProposals = [...state.peaceProposals, { from: state.currentPlayer, to: action.to }];
    return { ...state, peaceProposals };
}
function acceptPeace(state, action) {
    const peaceProposals = state.peaceProposals.filter(p => !(p.from === action.with && p.to === state.currentPlayer));
    const alliances = [...state.alliances, [action.with, state.currentPlayer]];
    return { ...state, peaceProposals, alliances };
}
function breakPeace(state, action) {
    const alliances = state.alliances.filter(([a, b]) => !((a === state.currentPlayer && b === action.with) || (a === action.with && b === state.currentPlayer)));
    return { ...state, alliances };
}
function exploreRuin(state, action) {
...
function buildRoad(state, action) {
    const unit = state.units.find((u) => u.id === action.unitId);
    const tile = state.tiles[unit.y * state.width + unit.x];
    const tiles = withTile(state.tiles, state.width, { ...tile, hasRoad: true });
    const players = state.players.map(p => p.id === state.currentPlayer ? { ...p, stars: p.stars - 1 } : p);
    return { ...state, tiles, players };
}
function upgradeHero(state, action) {
...
function removeUnit(units, tiles, width, dead, players) {
    ...
    if (dead.isHero && nextPlayers) {
        nextPlayers = nextPlayers.map(p => p.id === dead.ownerId ? { ...p, heroStatus: "dead" } : p);
    }
...
function attack(state, action) {
    ...
    let defenderNow = { ...defender, hp: defender.hp - result.defenderDamage };
...
function trainUnit(state, action) {
    ...
    let players = state.players.map((p) => p.id === city.ownerId ? { ...p, stars: p.stars - cost } : p);
    if (action.unitType === "hero") {
        players = players.map(p => p.id === city.ownerId ? { ...p, heroStatus: "alive" } : p);
    }
...
function moveUnit(state, action) {
    ...
    const moved = { ...unit, x: to.x, y: to.y, hasMoved: true, isEmbarked };
...
function isAlive(state, pid) {
    if (state.players[pid]?.civName === "Barbares")
        return true;
...
function endTurn(state) {
    ...
    const players = state.players.map((p) => {
        ...
        for (const c of state.cities) {
            if (c.ownerId === p.id) {
                const isNearOasis = state.tiles.some(t => t.naturalWonder === "oasis" && chebyshev(t, c) <= 2);
                if (isNearOasis)
                    cultureIncome += 5;
            }
        }
